    ...
    <div class="flex items-start gap-3">
      ...
      <div class="flex-1 min-w-0">
        <div class="flex items-center flex-wrap gap-x-2 gap-y-1 mb-1">
          ...
          <span v-if="comment.replyTo" class="text-xs text-gray-500">回复 @{{ comment.replyTo.username }}</span>
        ...
        <div class="flex items-center gap-3 mt-1.5">
          <button v-if="isLoggedIn" class="text-xs text-gray-400 hover:text-gray-700 transition-colors" @click="$emit('reply', comment.id, comment.user.username)">
            回复
          </button>
          <button v-if="canDelete" class="text-xs text-gray-400 hover:text-red-500 transition-colors" @click="$emit('delete', comment.id)">
            删除
          </button>
        </div>
      </div>
